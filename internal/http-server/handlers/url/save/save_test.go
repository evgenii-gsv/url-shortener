...
import (
	...
	"github.com/evgenii-gsv/url-shortener/internal/http-server/handlers/url/save/mocks"
...
func TestSaveHandler(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			cfgMock := config.Config{AliasLength: 6}
			...
			handler := save.New(slogdiscard.NewDiscardLogger(), urlSaverMock, &cfgMock)
